...
func setEnv() {
	// Set Ocean Server
	var OCEAN_SERVER string = strings.TrimSpace(os.Getenv("OCEAN_SERVER"))
	if len(OCEAN_SERVER) == 0 {
		os.Setenv("OCEAN_SERVER", defaultOceanServer)
	} else {
		if OCEAN_SERVER == "production" {
			// Production server
			// Set production variables
		} else if OCEAN_SERVER == "development" {
			// Development server
		} else if OCEAN_SERVER == "local" {
			// Local server
		} else {
			log.Warn("Invalid OCEAN_SERVER %s provided; using default %s instead", OCEAN_SERVER, defaultOceanServer)
			os.Setenv("OCEAN_SERVER", defaultOceanServer)
		}
	}

	pwd, _ := os.Getwd()
	os.Setenv("BASE_PATH", pwd+"/src/surf")
}
