...
func NewControl(ctlConn conn.Conn, authMsg *msg.Auth) {
	...
	// Validate auth token with dashboard first
	if valid, err := http.ValidateConnToken(authMsg.User, true); err != nil || !valid {
		failAuth(err)
		return
	}
	...
	if authMsg.Version != version.Proto {
		failAuth(fmt.Errorf("Incompatible versions. Server %s, client %s. Download a new version at http://getocean.io", version.MajorMinor(), authMsg.Version))
		return
	}
...
func (c *Control) registerTunnel(rawTunnelReq *msg.ReqTunnel) {
	for _, proto := range strings.Split(rawTunnelReq.Protocol, "+") {
		...
		failAuth := func(e error) {
			req := &msg.ReqTunnelResp{
				Error:     e.Error(),
				Version:   version.Proto,
				MmVersion: version.MajorMinor(),
				ClientId:  c.id,
			}
			_ = msg.WriteMsg(c.conn, req)
			c.conn.Close()
		}

		if validate, err := http.ValidateTunnelRequest(rawTunnelReq, c.auth.User, true); err != nil || !validate {
			failAuth(err)
			return
		}
